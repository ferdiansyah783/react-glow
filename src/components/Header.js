  ...
  return (
    <div className="w-full fixed top-0 z-[999]">
      <div className="relative flex justify-end">
        ...
        >
          <h1 onClick={() => scrollTo(homeRef.current)}>Home</h1>
          <h1 onClick={() => scrollTo(aboutRef.current)}>About</h1>
          <h1 onClick={() => scrollTo(portofolioRef.current)}>Portofolio</h1>
          <h1 onClick={() => scrollTo(contactRef.current)}>Contact</h1>
        </div>
      </div>
    </div>
  );
